                                                    ...
                                                    <div class="modal-dialog modal-xs">
                                                        <div class="modal-content">
                                                            ...
                                                            <div class="modal-body IR text-left">
                                                               ...
                                                               <div class="row">
                                                                    ...
                                                                    <div class="col-6">
                                                                        ...
                                                                        <form action="{{ route('admin.shop.sells.update', $order) }}" method="POST" class="d-inline" id="update{{ $order->id}}">
                                                                            @csrf
                                                                            @method('patch')
                                                                            <select name="delivery" id="delivery" class="dropdown" style="font-size:12px !important;">
                                                                                <option value="{{ App\Enums\DeliveryStatus::waiting}}" {{ App\Enums\DeliveryStatus::waiting==$order->delivery?'selected':'' }}>درانتظار</option>
                                                                                <option value="{{ App\Enums\DeliveryStatus::posted}}" {{ App\Enums\DeliveryStatus::posted==$order->delivery?'selected':'' }}>ارسال شده</option>
                                                                                <option value="{{ App\Enums\DeliveryStatus::delivery}}"  {{ App\Enums\DeliveryStatus::delivery==$order->delivery?'selected':'' }}>تحویل داده شده</option>
                                                                                <option value="{{ App\Enums\DeliveryStatus::repay}}"  {{ App\Enums\DeliveryStatus::repay==$order->delivery?'selected':'' }}>مرجوعی</option>
                                                                            </select>
                                                                        </form>
                                                                    </div>
                                                            ...
                                                            <div class="modal-footer">
                                                                @if(Auth::guard('admin')->user()->can('shop.products.sells.update') && $order->status == App\Enums\PaymentStatus::paid)
                                                                <button type="submit"  title="بروزرسانی" class="btn btn-info px-8"  form="update{{ $order->id}}">بروزرسانی</button>
                                                                @endif
                                                                &nbsp;
